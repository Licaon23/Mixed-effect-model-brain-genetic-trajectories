...
#===============================================================================
volVars <- sort(names(basal)[27:39])
PRSVars <- names(basal)[7:12]
RiskVars <- names(basal)[40:45]
snpVars <- names(basal)[13:23]
basal$tesla <- as.factor(basal$tesla)

# Select subset to analyse
...
adjustedGroupDifferences <- function(volume){
  ...
  expr <- paste(volume,"AGE + ICV + PTEDUCAT + PTGENDER * DX.bl + tesla",
                sep = " ~ ")
  ...
  # Marginal means
  RG <- ref_grid(mod,at = list(tesla ="3"))
...
cont <- lapply(cont,function(x)ifelse(x<0.001,"<0.001",sprintf("%.3f",x)))
...
taulaDef <- merge(taulaDef,pvalData,by="Volume",sort = F)
...
formattedTab <- footnote(tab,
                         general = c("Results are reported as estimated marginal mean volume and 95% confidence interval by diagnostic group and sex, adjusted by Age, Intracraneal total volume and tesla parameter.",
...
file <- "result_emmeansVolume.html"
path <- here("results",paramT,"2_Modelling",
             "1_baselineVolumes_groupAdjustedDifferences")
...
ggplot(emmeansData, aes(x=DX.bl)) + 
  ...
  labs(subtitle = "Adjusted for Age at baseline, sex, intracranial volume, diagnostic status, years of education and tesla parameter. <br/>
       <span style = 'color: #D95F02;'>**Female**</span> and <span style = 'color: #7570B3;'>**Male**</span>" ,
       caption="**Source**: ADNI project | **Units** Volume measurements in mm3 | Differences between sex are reported as significance level of t-test with Tukey's correction, sliced by diagnostic group, <br/>
       at levels \\*** <.001, \\** <.01, \\* <.05 and ·<.1. Non-significant differences are not shown.") +
...
effectPRS_strat <- function(volume,PRS,group="All"){
  data <- basal
  covars <- c("AGE","ICV","PTEDUCAT","PTGENDER","DX.bl","tesla")
...
modGrid <- expand.grid(PRS = c(PRSVars,snpVars),
                       Volume = volVars,
                       DX.bl = c("All",levels(basal$DX.bl)),
                       stringsAsFactors = F)

# Jerarquia analisis
modGrid$analysis[modGrid$PRS %in% c(PRSVars,"APOE4")] <- "Primary"
modGrid$analysis[modGrid$PRS %in% snpVars[-1]] <- "Secondary"

# modGrid$analysis[modGrid$PRS %in% PRSVars[1:4]] <- "Primary"
# modGrid$analysis[modGrid$PRS %in% c(PRSVars[5:6],"APOE4")] <- "Secondary"
...
names(auxQ1) <- sapply(auxQ1,function(x)
  paste(attributes(x$ModelGlobal)$PRS,
         attributes(x$ModelGlobal)$volume,
         sep="|")
)
...
modGrid$PRS <- factor(modGrid$PRS,levels=c(PRSVars,snpVars))
...
# modGrid$strat <- modGrid$DX.bl
# levels(modGrid$strat) <- c("All",rep("strat",3))
# res <- split(modGrid,modGrid$strat)
res <- split(modGrid,modGrid$DX.bl)


lr <- lapply(res, function(l){
  l$prsVol <- paste(l$PRS,l$Volume,sep="|")
  aux <- lapply(split(l,l$analysis), function(x){
    x$pvalue_corrected <- p.adjust(x$pvalue, method = "fdr")
...
res[paste(names(res)[11:12],"symbol",sep="_")] <-
  lapply(res[names(res[11:12])], FUN=cut,
         breaks = c(-Inf,0.001,0.01,0.05,0.1,1),
         labels=c("***","**","*","·",""))
 
significant_effects <- subset(res, pvalue_corrected_symbol!="")
...
# plotData <- subset(res,Volume!="Whole_hippocampus" & DX.bl=="All")
# plotData <- subset(res,& DX.bl=="All")
...
heatmapT1 <- ggplot(data=plotData,aes(x=Volume,y=PRS,fill=Effect)) + 
  ...
  ggtitle("Corrected significant associations between PRS or individual SNPs and
          hippocampal subfield volumes") +
  labs(caption="PRS linear effects on hippocampal volumes, which corresponds to beta1 coefficient of T.1 models. Pvalues are reported as \\*** <.01\\** <.01, \\* <.05 and<br/>· <.1.
  after correcting for multiple comparisons. Effects adjusted for Age at baseline, intracranial volume, sex, diagnostic status, years of education and<br/>
  tesla scan parameter. **Source**: ADNI project") +
  theme(plot.caption = element_markdown(hjust = 0, lineheight = 1.5),
        plot.subtitle = element_markdown(size = 12, hjust = 0),
...
heatmapT1
subset(res, pvalue_symbol !="" & DX.bl=="All",
       c(Volume,PRS,Effect,pvalue_symbol,pvalue_corrected_symbol))


### All+Groups: primary analysis
plotData <- subset(res,analysis=="Primary")
names(plotData)[3] <- "Group"

upper.th <- quantile(plotData$Effect,1)
lower.th <- quantile(plotData$Effect,0)
plotData$Volume <- factor(plotData$Volume,levels=unique(plotData$Volume),
                          labels=c("Whole\nhippocampus","Subiculum","Pre-\nsubiculum","Para-\nsubiculum"))


heatmap <- ggplot(data=plotData,aes(x=Volume,y=PRS,
                                    fill=pmax(pmin(Effect,upper.th),lower.th))) + 
  ...
  # ggtitle("Corrected significant associations between PRS or individual SNPs and
  #         hippocampal subfield volumes") +
  labs(caption="Genetic predictor linear main effects on hippocampal volumes. Pvalues are reported as \\*** <.01\\** <.01, \\* <.05 and ·<.1.<br/>
  after correcting for multiple comparisons. Effects adjusted for Age at baseline, intracranial volume, sex, diagnostic status,<br/>
       years of education and tesla scan parameter.") +
  theme(plot.caption = element_markdown(hjust = 0, lineheight = 1.5),
        plot.subtitle = element_markdown(size = 12, hjust = 0),
        plot.title = element_text(size = 16, hjust = 0))
...
heatmap.primary
#8.68x5.45 in  export pdf



### All: secondary analysis
plotData <- subset(res,analysis=="Secondary")
names(plotData)[3] <- "Group"
...
heatmap.secondary
#8.68x6 in  export pdf



### All + groups, primary analysis, only Whole hippocampus
plotData <- subset(res,analysis=="Primary" & Volume == "Whole_hippocampus")
names(plotData)[3] <- "Group"
...
heatmap <- ggplot(data=plotData,aes(x=Group,y=PRS,
                                    fill=pmax(pmin(Effect,upper.th),lower.th))) + 
  geom_tile(color="grey") +
  geom_text(aes(label=pvalue_corrected_symbol),size=4)+
  scale_fill_gradient2(low="red4",mid = "white", high="#0066FF", midpoint = 0,
...
#-----------------------------------------------------------------------------
volume <- "CA1"; PRS <- "AD_predisposition"; interact <- "DX.bl"
...
modGridQ2 <- subset(modGrid,DX.bl=="All",select=c(PRS,Volume,analysis))
...
aux <- lapply(betaPRS_DXInt, function(x){
  ...
  betas <- df[,c(2:7)]
  names(betas)[c(2,6)] <- c("Effect","pvalue")
...
resultQ2$PRS <- factor(resultQ2$PRS,levels=c(PRSVars,snpVars))
resultQ2$DX.bl <- factor(resultQ2$DX.bl, levels=c("CN","MCI","AD"))
resultQ2$analysis <- modGridQ2$analysis[match(resultQ2$PRS,modGrid$PRS)]

# Corregim pvalors segons FDR, considerant cada volum independent,
#  i corregint per 6 comparacions (6 PRS) o models per cada un dels volums
resultQ2$prsVolDX <- paste(resultQ2$PRS,resultQ2$Volume,resultQ2$DX.bl,sep="|")
aux <- lapply(split(resultQ2,resultQ2$analysis), function(x){
  x$pvalue_corrected <- p.adjust(x$pvalue, method = "fdr")
  return(x[c("prsVolDX","pvalue_corrected")])
})
aux <- do.call(rbind,aux)
pos <- match(resultQ2$prsVolDX,aux$prsVolDX)
resultQ2$pvalue_corrected <- aux[pos,"pvalue_corrected"]
resultQ2$prsVolDX <- NULL


# Afegim simbols de significació
resultQ2[paste(names(resultQ2)[c(8,10)],"symbol",sep="_")] <-
  lapply(resultQ2[names(resultQ2[c(8,10)])], FUN=cut,
         breaks = c(-Inf,0.001,0.01,0.05,0.1,1),
         labels=c("***","**","*","·",""))

# PLOT DATA····
resultQ2.withAll <- resultQ2
aux.all <- res[res$DX.bl=="All",names(resultQ2.withAll)]
resultQ2.withAll <- rbind(resultQ2.withAll,aux.all)
resultQ2.withAll$DX.bl <- relevel(resultQ2.withAll$DX.bl,ref="All")
names(resultQ2.withAll)[3] <- "Group"

# Primary
plotData <- resultQ2.withAll[resultQ2.withAll$analysis=="Primary",]
upper.th <- quantile(plotData$Effect,1)
lower.th <- quantile(plotData$Effect,0)
plotData$Volume <- factor(plotData$Volume,levels=unique(plotData$Volume),
                          labels=c("Whole\nhippocampus","Subiculum","Pre-\nsubiculum","Para-\nsubiculum"))
...
heatmap <- ggplot(data=plotData,aes(x=Volume,y=PRS,
                                    fill=pmax(pmin(Effect,upper.th),lower.th))) + 
  geom_tile(color="grey") +
  geom_text(aes(label=pvalue_corrected_symbol),size=5)+
  scale_fill_gradient2(low="red4",mid = "white", high="#0066FF", midpoint = 0,
                       name="Effect")+
  facet_wrap(~Group)+
  ggstatsplot::theme_ggstatsplot() +
...
### All: secondary analysis
plotData <- resultQ2.withAll[resultQ2.withAll$analysis=="Secondary",]
upper.th <- quantile(plotData$Effect,1)
lower.th <- quantile(plotData$Effect,0)
plotData$Volume <- factor(plotData$Volume,levels=unique(plotData$Volume),
                          labels=c("Whole\nhippocampus","Subiculum","Pre-\nsubiculum","Para-\nsubiculum"))
...
# Primary Whole hippocampus
plotData <- resultQ2.withAll[resultQ2.withAll$analysis=="Primary" &
                             resultQ2.withAll$Volume=="Whole_hippocampus",]
upper.th <- quantile(plotData$Effect,1)
lower.th <- quantile(plotData$Effect,0)
...
### Secondary analysis - Whole hippo
plotData <- resultQ2.withAll[resultQ2.withAll$analysis=="Secondary" & 
                             resultQ2.withAll$Volume=="Whole_hippocampus",]
upper.th <- quantile(plotData$Effect,1)
lower.th <- quantile(plotData$Effect,0)
...
heatmap.secondary.WH <- heatmap
heatmap.secondary.WH
...
heatmap.primary.subfields <- heatmap
heatmap.primary.subfields
...
heatmap.secondary.subfields <- heatmap
heatmap.secondary.subfields
...
plotData <- subset(res,Volume!="Whole_hippocampus")
names(plotData)[3] <- "Group"
heatmapT1 %+%plotData +
  labs(caption="PRS linear effects on hippocampal volumes, which corresponds to beta1 coefficient of T.1 models. Pvalues are reported as \\*** <.001, \\** <.01,<br/>
  \\* <.05 and · <.1 after correcting for multiple comparisons. Effects adjusted for Age at baseline,<br/>
  sex, intracranial volume, years of education and tesla scan parameter.<br/>
  **Source**: ADNI project")
...
effectPRS_strat_inter <- function(volume,PRS,group){
  ...
  covars <- paste(c("AGE","ICV","PTEDUCAT","tesla","PTGENDER"),
                  collapse=" + ")
...
# Provar funció...
#effectPRS_strat_inter("Hippocampal_tail","PRS_AD",group = "AD")

### i) Construcció matriu combinacions PRS, volums i estrat DX.bl
modGrid_strata <- expand.grid(PRS = c(PRSVars,snpVars),
                              Volume = sort(volVars),
...
effectPRS_Sx$alpha <- with(effectPRS_Sx,ifelse(Significance<0.05,1,.9))
...
effectPRS_Sx$analysis <- modGrid$analysis[match(effectPRS_Sx$PRS,modGrid$PRS)]

# v) PANEL DE RESULTATS PRIMARI
...
# Auxiliar
  ...
  PRS.labs <- c(sapply(strsplit(PRSVars,split="_"),"[",2),"APOE4")
  names(PRS.labs) <- c(PRSVars,"APOE4")
  PRS.labs[PRS.labs=="ADnoAPOE"] <- "AD\nno APOE"

# PANEL
plotData <- effectPRS_Sx[effectPRS_Sx$analysis=="Primary",]  
plotData$Volume <- factor(plotData$Volume, levels=volVars)  
ggplot(plotData,aes(x=DX.bl)) +
...
#           position = position_dodge(width=0.1), show.legend = F)+
  ...
  labs(subtitle = "Adjusted for Age at baseline, sex, intracranial volume, years of education and tesla scan parameter. <br/>
       ...
       caption="**Source**: ADNI project | **Units** Linear coefficient and 95% confidence intervals of PRS effect, for female and male when sex interaction with<br/>
       PRS is significant; or main effect otherwise in models T.3. Non-significant coeficients are shown with transparency, according to Wald-test<br/>
       p-value at level 0.05.") +
  theme(plot.caption = element_markdown(hjust = 0, lineheight = 1.5),
        plot.subtitle = element_markdown(size = 12, hjust = 0),
        plot.title = element_markdown(size = 16, hjust = 0)) -> primaryPanel

primaryPanel
# Export 9.86x7.58 landscape

# v) PANEL DE RESULTATS Secundari eqtl
...
# PANEL
effectPRS_Sx[grep("eQTL",effectPRS_Sx$PRS),"analysis"] <- "Secondary.eQTL"
plotData <- effectPRS_Sx[effectPRS_Sx$analysis=="Secondary.eQTL",]  
plotData$Volume <- factor(plotData$Volume, levels=volVars)  
...
ggplot(plotData, aes(x=DX.bl)) +
  ...
  theme(strip.background = element_blank(),
        ...
        axis.title = element_blank(),
        axis.text.x = element_text(angle=90,hjust=1)) +
  ggtitle("Association results for direct effects of polygenic risk scores on hippocampal<br/>volumes for female and male and stratified by diagnostic group.") +
  labs(subtitle = "Adjusted for Age at baseline, sex, intracranial volume, years of education and tesla scan parameter. <br/>
       <span style = 'color: #000000;'>**Global**</span> , <span style = 'color: #D95F02;'>**Female**</span> and <span style = 'color: #7570B3;'>**Male**</span>" ,
       caption="**Source**: ADNI project | **Units** Linear coefficient and 95% confidence intervals of PRS effect, for female and male when sex<br/>
       interaction with PRS is significant; or main effect otherwise in models T.3. Non-significant coeficients are shown with<br/>
       transparency, according to Wald-test p-value at level 0.05.") +
  theme(plot.caption = element_markdown(hjust = 0, lineheight = 1.5),
        plot.subtitle = element_markdown(size = 12, hjust = 0),
        plot.title = element_markdown(size = 16, hjust = 0)) -> secondaryPanel.eQTL

# Export 9.86x7.58 portrait
secondaryPanel.eQTL


# v) PANEL DE RESULTATS Secundari pQTL
...
# PANEL
effectPRS_Sx[grep("pQTL",effectPRS_Sx$PRS),"analysis"] <- "Secondary.pQTL"
plotData <- effectPRS_Sx[effectPRS_Sx$analysis=="Secondary.pQTL",]  
plotData$Volume <- factor(plotData$Volume, levels=volVars)  
...
ggplot(plotData, aes(x=DX.bl)) +
  ...
  theme(strip.background = element_blank(),
        ...
        axis.title = element_blank(),
        axis.text.x = element_text(angle=90,hjust=1)) +
  ggtitle("Association results for direct effects of polygenic risk scores on hippocampal<br/>
          volumesfor female and male and stratified by diagnostic group.") +
  labs(subtitle = "Adjusted for Age at baseline, sex, intracranial volume, years of education and tesla scan parameter. <br/>
       <span style = 'color: #000000;'>**Global**</span> , <span style = 'color: #D95F02;'>**Female**</span> and <span style = 'color: #7570B3;'>**Male**</span>" ,
       caption="**Source**: ADNI project | **Units** Linear coefficient and 95% confidence intervals of PRS effect, for female and male when sex<br/>
       interaction with PRS is significant; or main effect otherwise in models T.3. Non-significant coeficients are shown with<br/>
  ...
  theme(plot.caption = element_markdown(hjust = 0, lineheight = 1.5),
        plot.subtitle = element_markdown(size = 12, hjust = 0),
        plot.title = element_markdown(size = 16, hjust = 0)) -> secondaryPanel.pQTL
...
prsEffectList <- list("Models T1" = subset(res,DX.bl=="All",c(2,1,4:8,11,12)),
                      "Models T2" = resultQ2[c(9,1:8,10)],
