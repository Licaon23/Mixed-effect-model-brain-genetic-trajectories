...
# SNPs
aux <- grep("^(eQTL|pQTL)",names(patientInfo))
snpVars <- names(patientInfo)[aux]
...
volumes <- merge(volumes,
                 patientInfo[,c("PTID","enrolDate","AGE","PTGENDER","PTEDUCAT","DX.bl","ATgroup",
                                "APOE4",PRSVars, snpVars,
                                "Path")], 
                 by="PTID")
...
# Reorder variables
volumes <- volumes[,c("PTID","AGE","DX.bl","PTGENDER","PTEDUCAT","ATgroup",
                      PRSVars,"APOE4",snpVars,
                      "measureDate","enrolDate","timelapse","VISCODE2",
...
# =============================================================================
  ...
  aux <- lapply(names(basal)[c(2,37:63)], FUN=testMissingEfect,
...
#  sum(exclude1) # 588 patients to be excluded
  ...
  missingEffect
    ...
    # Nevertheless, it is necessary to exclude them.
...
volumes_singleT <- lapply(volumes_singleT, function(x){
  ...
  x <- x[,c("PTID","AGE","DX.bl","PTGENDER","PTEDUCAT","ATgroup",
            PRSVars,"APOE4",snpVars,
            "enrolDate","Path",
...
# v <- names(dd1)[grep(pattern = paste0("(left|right)_",measure),names(dd1))]
# 
# check <- subset(dd1,PTID==ptid,select=c("PTID","VISCODE2","time_visit","time_age",v))
# check <- merge(check,dd2[,c("PTID","VISCODE2",measure)],by=c("PTID","VISCODE2"))
# check$check <- identical(check[,ncol(check)-2]+check[,ncol(check)-1],check[,ncol(check)])
# check
...
codebookBoth$Label <- c("Patient ID",
                     ...
                     "AT status",
                     paste("PRS",c("AD","AD_noAPOE","LONGEVITY","LONGEVITYnoAPOE",
                                   "FTD","PKSON","EEAA","IEAA","FRAILTY","LEXP","TELOM")),
                     "PRS brain eQTL for APOE",
                     "PRS plasma pQTL for APOE",
                     "APOE4 carriership",
                     codebookBoth$Variable[21:30],
                     "Date of enrollment",
                     "ADNI phases path",
...
codebookBoth$description <- c("Patient identification number",
                           ...
                           rep("Number of effect allelles for a given SNP (0,1,2)",10),
