...
catSummary <- function(x, xname, dig=1, data){
  ...
  if(length(levels(x))!=2 | levels(x)[1] == "Female"){
...
#====================================================================
numSummary <- function(x, xname=NULL, dig=1, data, nonNormal=NA){
  if(is.null(xname)){xname <- x}
  ...
  if(is.na(nonNormal)){
    ...
    nonNormal <- normalityTest$p.value < 0.05
  }
...
#====================================================================
summaryWrapper <- function(x, xname=NULL, dig=1, nonNormal = NA, data){
  
  #Numeric
  if(is.numeric(data[[x]])){res <- numSummary(x,xname,dig,data, nonNormal)}
...
#====================================================================
descSummary <- function(data, vars, printNames=NULL, nonNormal = NULL, dig=1){
  ...
  if(is.null(nonNormal)){nonNormal <- rep(NA,length(vars))}

    
  varNames <- data.frame(x=vars, xnames=printNames, nonNormal=nonNormal)
  res <- lapply(varNames$x, 
                function(x){summaryWrapper(x=x,
                            xname = varNames[which(varNames$x==x),"xnames"],
                            dig=dig,
                            nonNormal = varNames[which(varNames$x==x),"nonNormal"],
                            data=data)})
...
#====================================================================
groupContrast <- function(x,nonNormal=NA,by,data){
  ...
  if(is.factor(data[[x]])){
    ...
    if(nlevels(data[[x]])!=2 | levels(data[[x]])[1] == "Female"){
      pval <- c(pval,rep("",nlevels(data[[x]])))
    }  
  }
